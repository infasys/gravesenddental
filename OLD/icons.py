...
class ScrollWidget(QWidget):
    ...
    def __init__(self, contentWidget: QWidget):
        super().__init__()
...
class IconWidget(QWidget):
    ...
    def getLabelWithIcon(self, icon: QIcon):
        lab = QLabel(self)
        lab.setPixmap(icon.pixmap(16, 16))
        return lab


class AllIconsWidget(IconWidget):
...
class FreeDesktopWidget(IconWidget):
    """
    ...
    def __init__(self, name, parent=None):
        ...
        self._layout.addWidget(self.label, 0, 0, 1, 3)

    def addIcon(self, iconName, description=''):
        currentRow = self._layout.rowCount()

        le = QLineEdit(iconName, self)
        le.setReadOnly(True)
        self._layout.addWidget(le, currentRow, self.NAME_COLUMN)

        icon = QIcon.fromTheme(iconName)
        self._layout.addWidget(self.getLabelWithIcon(icon), currentRow, self.ICON_COLUMN)

        des = QLabel(description, self)
        des.setToolTip(description)
        self._layout.addWidget(des, currentRow, self.DESCRIPTION_COLUMN)
...
class MyWidget(QWidget):
    def __init__(self, parent=None):
        ...
        self.tabWidget.addTab(ScrollWidget(AllIconsWidget()), 'Qt icons')
        for w in self.generateFreeDesktopWidgets():
            self.tabWidget.addTab(ScrollWidget(w), w.name)

    @staticmethod
    def generateFreeDesktopWidgets():
        lastWidget = None
        for line in iconsAndDescriptions.splitlines():
            ...
            if lastWidget is None:
                lastWidget = FreeDesktopWidget(line)
            else:
                pos = line.find(' ')
                lastWidget.addIcon(line[:pos], line[pos + 1:])
